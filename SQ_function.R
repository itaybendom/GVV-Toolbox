# 29/11/16
#
# Speed Quotient is defined as the ratio of the open phase to the closing phase
...
# 22/8/2016
# This current implementaion is different than OQpoints.R although some similarities are present i.e. the detection of the "rough"
# GOI/GCI points location.

# 2/9/2016
# This is a modification of the OQpoints_tolerance.R function (also OQ50). In this version, all steps are the same except for the GCI
# detection method which is based on the GVV waveform derivative and involves interpolation rather than polynomial fitting.
#
# Created 2/9/2016
# By Itay Ben-Dom
###############################################################################################################################################
...
SQ_function <- function(glottalList, threshold = 0.5){
  ...
  for (i in 1:nrow(results_df)) {
    ...
    GCItmp = GCI_instant(results_df[i, ], winwave,  i, showPlot = F)
    if (length(GCItmp > 0)) {
      GCI_list[i] = GCItmp
    } else{
      if (flag != 1)
  ...
  ############################
  # Speed QUOTIENT Calculation
  ############################
  
  for (i in 1:nrow(results_df)) {
    SQ = (results_df$`Peak Idx`[i] - results_df$GOI[i]) / (results_df$GCI[i] - results_df$`Peak Idx`[i])
    results_df[i, 7] = SQ
    rm(SQ)
  }
  names(results_df)[7] = "SQ"
  
  # Return SQ values
  # Vaector of OQ values is outputted as list object. This allows the values to be inserted into another list
  # and then analyzed using lapply(). results_df$SQ is outputted as a list to mimic the output of trapply().
  return(list(results_df$SQ))
